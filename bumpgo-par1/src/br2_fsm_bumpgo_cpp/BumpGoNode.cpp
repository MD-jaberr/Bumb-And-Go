...
int preferred_turn_dir_ = -1; // Preferred Turning Direction to use as a default
...
BumpGoNode::check_forward_2_back()
{
  ...
  for (int i = 196; i <= 470; ++i)
  {
    ...
    {
      if (i < 333) {  // Right half of the ±45° cone
        min_range_right = std::min(min_range_right, range);
      } else {        // Left half of the ±45° cone
  ...
  {
    preferred_turn_dir_ = (min_range_left > min_range_right) ? 1 : -1;
    return true;
  }
